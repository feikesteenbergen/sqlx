...
impl<'a> ParsedQuery<'a> {
    ...
    pub fn expand<DB: Database>(
        &self,
        get_arg: impl FnMut(&ArgumentIndex<'_>, &Placeholder<'a>) -> Result<ArgumentKind, String>,
    ) -> Result<Cow<'a, str>> {
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ArgsMismatch(s) => write!(f, "Error matching placeholders to arguments: {}", s),
            Error::PlaceholderToArgument { argument, message } => {
